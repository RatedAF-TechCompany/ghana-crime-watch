...
export default function FraudWatchHomeView() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      router.push(`/fraud-watch/search?q=${encodeURIComponent(searchQuery.trim())}`);
    }
  };
        ...
        <div className="grid md:grid-cols-5 gap-6">
          ...
          <div className="md:col-span-3">
            <div className="flex items-center justify-between mb-4 border-b-2 border-primary pb-2">
              ...
              <Link href="/fraud-watch/search?status=VerifiedScam" className="text-sm text-primary hover:underline flex items-center gap-1">
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {latestScams.map((account) => (
                  <Link key={account.id} href={`/fraud-watch/account/${account.id}`} className="block">
            ...
            {loading ? (
              ...
              <ol className="space-y-2">
                {topSearched.map((account, idx) => (
                  <li key={account.id}>
                    <Link
                      href={`/fraud-watch/account/${account.id}`}
