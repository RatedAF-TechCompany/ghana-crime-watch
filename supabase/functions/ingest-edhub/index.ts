...
const EDHUB_USERNAME = "eddie_wrt";
...
// OAuth 1.0a helpers (reused from auto-tweet)
...
function isCrimeRelated(text: string): boolean {
  ...
  // Must contain at least one crime keyword
  return CRIME_KEYWORDS.some((kw) => lower.includes(kw));
}

// Known acronyms to preserve during casing
...
function sentenceCaseWithAcronyms(text: string): string {
  ...
  // Apply multi-word proper nouns (longest first)
...
Return ONLY the tweet text.`
      ...
      if (res.ok) {
        ...
        let rewritten = data.choices?.[0]?.message?.content?.trim()?.replace(/^["']|["']$/g, "");
        if (rewritten) {
...
serve(async (req) => {
  ...
  try {
    // Step 1: Get @eddie_wrt user ID
    const userLookupUrl = "https://api.x.com/2/users/by/username/" + EDHUB_USERNAME;
    ...
    console.log(`Fetched ${tweets.length} tweets from @${EDHUB_USERNAME}`);
    ...
    for (const tweet of newTweets) {
      ...
      await supabase.from("processed_tweets").insert({
        tweet_id: tweetId,
        author_username: EDHUB_USERNAME,
      ...
      try {
        const aiPrompt = `You are a professional news editor for GhanaCrimes.com. Convert this tweet from EDHUB (@eddie_wrt) into a short crime news article.
...
- Headline must follow HOOK + ACTION + CRIME/EVENT + LOCATION pattern`;
        ...
        // Mark processed tweet as published
        await supabase.from("processed_tweets")
          .update({
    ...
    console.error("EDHUB ingestion error:", errMsg);
